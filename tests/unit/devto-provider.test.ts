...
describe('DevToProvider published', () => {
  afterEach(() => {
    delete process.env.DEVTO_PUBLISHED;
    vi.unstubAllGlobals();
  });
  ...
  it('creates a draft by default', async () => {
    ...
    await provider.sync(makeCtx({ deployments: {} }));

    const [, init] = fetchMock.mock.calls[0];
    expect(JSON.parse(init.body).article.published).toBe(false);
  });

  it('creates a live post when `published: true` is passed explicitly', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ id: 1, url: 'https://dev.to/x/1' }));
    vi.stubGlobal('fetch', fetchMock);

    const provider = new DevToProvider({ apiKey: 'key', published: true });
    await provider.sync(makeCtx({ deployments: {} }));

    const [, init] = fetchMock.mock.calls[0];
    expect(JSON.parse(init.body).article.published).toBe(true);
  });

  it('falls back to DEVTO_PUBLISHED=true when `published` is omitted', async () => {
    process.env.DEVTO_PUBLISHED = 'true';
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ id: 1, url: 'https://dev.to/x/1' }));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('never sends `published` on an update, even when configured to publish on create', async () => {
    ...
    const [, init] = fetchMock.mock.calls[0];
    expect(JSON.parse(init.body).article).not.toHaveProperty('published');
  });
});
