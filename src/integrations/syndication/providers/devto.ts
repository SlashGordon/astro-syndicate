import type { BlogPost, SyncContext, SyncResult, SyndicationProvider } from '../types.js';
...
export interface DevToProviderOptions {
  ...
  /**
   * Publish new posts live instead of creating them as a draft.
   * Falls back to `DEVTO_PUBLISHED=true` when omitted; defaults to `false`
   * (draft) otherwise. Only ever applies to a newly *created* post - see the
   * CRITICAL note below for why an update never touches `published`.
   */
  published?: boolean;
}
...
/**
 * CRITICAL: `published` is only ever sent on creation.
 *
 * Per the dev.to API docs, sending `published: false` on an UPDATE to an
 * already-published article silently reverts it to draft - so once a post
 * goes live (by hand, or via `DevToProviderOptions.published`), this
 * integration must never touch that field again, or the next content edit
 * would unpublish it.
 * https://developers.forem.com/api/v1#tag/articles/operation/updateArticle
 */
interface DevToCreateBody {
  article: DevToArticleFields & { published: boolean };
}
...
export class DevToProvider implements SyndicationProvider {
  ...
  constructor(options: DevToProviderOptions = {}) {
    ...
    this.#published = options.published ?? process.env.DEVTO_PUBLISHED === 'true';
  }
  ...
  public async sync(ctx: SyncContext): Promise<SyncResult> {
    ...
    if (typeof remoteId !== 'number') {
      const body: DevToCreateBody = { article: { ...fields, published: this.#published } };
